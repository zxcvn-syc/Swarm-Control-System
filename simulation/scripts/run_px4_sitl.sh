...
#   3. Starts the single PX4 instance supported by this reproducible profile.
...
#                         contain build/px4_sitl_default/bin/px4 (PX4 v1.14)
#                         and etc/px4-rc.mavlink.
#   NUM_UAV              Must be 1 for this profile. Default: 1.
...
if [[ "$NUM_UAV" != "1" ]]; then
    echo "[run_px4_sitl] this reproducible profile supports NUM_UAV=1" >&2
    exit 1
fi
...
PX4_BUILD_DIR="$PX4_SITL_ROOT/build/px4_sitl_default"
PX4_SITL_BIN="$PX4_BUILD_DIR/bin/px4"
if [[ ! -x "$PX4_SITL_BIN" ]]; then
    PX4_SITL_BIN="$PX4_BUILD_DIR/px4_sitl_default"
fi
SITL_GAZEBO_DIR="$PX4_SITL_ROOT/Tools/simulation/gazebo-classic/sitl_gazebo-classic"
SITL_RUN="$PX4_SITL_ROOT/Tools/simulation/gazebo-classic/sitl_run.sh"
...
if [[ ! -x "$SITL_RUN" ]]; then
    echo "[run_px4_sitl] PX4 Gazebo launcher not found at $SITL_RUN" >&2
    exit 2
fi
...
echo "[run_px4_sitl] starting one PX4 SITL instance with Gazebo Classic..."
(
    "$SITL_RUN" \
        "$PX4_SITL_BIN" \
        none \
