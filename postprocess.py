"""Post-process the tracker's per-frame CSV into per-track trajectories and a
quick matplotlib overlay figure.

Outputs:
    output/tracks_per_id.csv  : long-form id/frame/x/y/vx/vy/label
    output/tracks_summary.csv : one row per id (start_frame, end_frame, length,
                                distance, avg_speed, label)
    output/trajectories.png   : 2D trajectories with GT overlay (if available)
"""
...
def plot_trajectories(by_id, gt_path, out_png):
...
def main() -> None:
    ...
    ap.add_argument("--tracks",
                    default="/home/hhh/Downloads/cv_tracking_demo/tracks.csv")
    ap.add_argument("--gt",
                    default="/home/hhh/Downloads/cv_tracking_demo/sample_gt.csv")
    ap.add_argument("--out-dir",
                    default="/home/hhh/Downloads/cv_tracking_demo/output")
    args = ap.parse_args()
    ...
    if by_id:
        ...
        # write a placeholder PNG so downstream tooling doesn't break
        import matplotlib.pyplot as plt
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.text(0.5, 0.5, "no tracks produced",
