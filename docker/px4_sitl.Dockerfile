# ROS2 Humble + Gazebo Classic + PX4 SITL simulation image.
#
...
RUN apt-get update && apt-get install -y --no-install-recommends \
        ...
        ros-humble-tf2-ros \
    && pip3 install --no-cache-dir "numpy<2" "opencv-python<4.11" \
...
RUN apt-get update && apt-get install -y --no-install-recommends \
# Gazebo Classic is the supported PX4 v1.14 backend.
